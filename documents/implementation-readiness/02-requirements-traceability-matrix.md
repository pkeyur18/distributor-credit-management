...
| FR-3 | Member detail: name, phone, address, full Rewards breakdown, direct children (1 depth), TBV, leg count | UN-17 | M4 | Member Detail | `get_member_detail` | members, member_period_totals | TEST-FR3 | Fully traced | Amended 8 Aug 2026 by CR-4 — Rewards breakdown gains an own-Business-Volume reward line, shown first. See Rule-46 below. |
| Rule-46 | Own Business Volume also earns, at the member's own slab: `OwnReward(x) = slab%(x) × BusinessVolume(x)`. Additive — Differential/Royalty unchanged | (client change request) | M3 | Member Detail (Rewards breakdown, own-BV line first), Home (Rewards-by-slab chart) | `get_member_detail` | member_period_totals.own_reward | TEST-R46 | Fully traced | New 8 Aug 2026, CR-4. Reverses the 3 Aug 2026 position that own Business Volume earns nothing — only for this new term; Differential (Rule-8) and Royalty (Rule-10) keep that exclusion. Golden regression set: scenarios 1–3 recomputed (65/62/510), 4–5 unchanged (1,000/980, own BV = 0 there), new scenario 6 (total 10). |
| — | Home shows a "Rewards by slab" chart alongside "Members by slab" — same bar-per-slab pattern, summing Rewards instead of counting members, current live period only | (client change request) | M4 | Home | (client-side aggregation of already-fetched period figures — same as the members-by-slab chart, which also has no dedicated command) | member_period_totals | TEST-R46-CHART | Fully traced | New 8 Aug 2026, CR-5. No new API command — mirrors the sibling chart's existing pattern. |
...
| Rule-21 | Period = calendar month; reset closes whichever month it belongs to | UN-18 | M5 | Monthly Close wizard | `begin_close` | periods | TEST-R21 | **Partially traced** | 3rd bullet (late entries fall into the closing month) stays struck through — an entry counts into the month its own date falls in, never into "the month being closed". Originally unreachable under Rule-36's hard lock; since Rule-36's 7 Aug 2026 amendment (CR-2) the scenario is reachable again (an entry can be made between the 1st and the reset), but the bullet's rule is still wrong and must still not be implemented. Retained in the spec for historical record. |
...
| Metric | Count |
|---|---|
| Total requirement-level items (Rules + FR + new items) | 57 |
| Non-functional requirements | 16 |
| User needs | 31 |
| **Fully traced** | 51 Rules/FR items, all 16 NFRs, all 31 UNs |
...
Further changes on 8 August 2026 — client change requests **CR-4, CR-5** (see [../final/06-decision-log-and-open-items.md](../final/06-decision-log-and-open-items.md) §5):
- **Added:** Rule-46 (own-Business-Volume reward) and a row for the Home "Rewards by slab" chart — taking requirement-level items from 55 to 57.
- **Amended:** FR-3's row (Rewards breakdown gains an own-Business-Volume line), Rule-12 (Rewards formula, cited wherever the calculation engine is referenced).
- **No new API command.** API-10 (`get_member_detail`) gains one response field; the Home chart is client-side aggregation, same as its sibling "Members by slab" chart, which also has no dedicated command. The surface stays at 40.

No orphan rows: every Rule 1–46, every FR-1–10, every UN-01–31, and every NFR has at least one entry above.
